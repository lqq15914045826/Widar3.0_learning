...
[~,idx] = max(freq_bin);
circ_len = length(freq_bin) - idx; %计算移位步长
doppler_spectrum = circshift(doppler_spectrum, [0 circ_len 0]); %循环移位函数
...
for ges_number = 1:ges_per_file
    ...
    % Save VS
    save([dpth_vs, spfx_ges, '-', num2str(ges_number),...
