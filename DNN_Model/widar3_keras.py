...
data_dir = "Widar3.0Release-Matlab/BVP/"
...
def normalize_data(data_1):  # 对每个时间帧做min-max归一化
    # data(ndarray)=>data_norm(ndarray): [20,20,T]=>[20,20,T]
    data_1_max = np.concatenate((data_1.max(axis=0), data_1.max(axis=1)), axis=0).max(
        axis=0
    )
    data_1_min = np.concatenate((data_1.min(axis=0), data_1.min(axis=1)), axis=0).min(
        axis=0
    )
...
def zero_padding(data, T_MAX):  # 填充
    ...
    for i in range(len(data)):
        ...
        data_pad.append(
            np.pad(
                data[i], ((0, 0), (0, 0), (T_MAX - t, 0)), "constant", constant_values=0
            ).tolist()
        )
...
def load_data(path_to_data, motion_sel):
    ...
    # Swap axes
    data = np.swapaxes(
        np.swapaxes(data, 1, 3), 2, 3
    )  # [N,20,20',T_MAX]=>[N,T_MAX,20,20']
...
def assemble_model(
    input_shape, n_class
):  # assemble组装_model(input_shape=(T_MAX,20,20,1), n_class=N_MOTION)

    model_input = Input(
        shape=input_shape, dtype="float32", name="name_model_input"
    )  # Input (@,T_MAX,20,20,1)
    ...
    x = TimeDistributed(
        ...
        model_input
    )  # (@,T_MAX,20,20,1)=>(@,T_MAX,16,16,16)
    x = TimeDistributed(MaxPooling2D(pool_size=(2, 2)))(
        x
    )  # (@,T_MAX,16,16,16)=>(@,T_MAX,8,8,16)
    x = TimeDistributed(Flatten())(x)  # (@,T_MAX,8,8,16)=>(@,T_MAX,8*8*16)
    x = TimeDistributed(Dense(64, activation="relu"))(
        x
    )  # (@,T_MAX,8*8*16)=>(@,T_MAX,64)
    ...
    model.compile(
        optimizer=keras.optimizers.RMSprop(lr=f_learning_rate),
...
print(
    "\nTrain on "
    + str(label_train.shape[0])
    + " samples\n"
    + "Test on "
    + str(label_test.shape[0])
    + " samples\n"
)
